...
description: >
  ...
  versions, upgrading AKS add-ons (monitoring, azure-keyvault-secrets-provider, ingress-appgw,
  azure-policy, gitops, open-service-mesh, virtual-node), upgrading AKS extensions, upgrading
  Helm releases on AKS, checking AKS/add-on compatibility, or planning a control-plane minor bump —
  even without the word "AKS" when context (az aks, azurerm_kubernetes_cluster, terraform
  `azurerm_aks_*`) is clear.
---
...
The goal of this follow-up is to know, for each module call: *(a)* the latest available
module version, *(b)* which input variable maps to the K8s control-plane version, and
*(c)* which input controls each add-on/extension version — so Phase 4 can plan an exact edit.
...
- The command that *would* be run (but do not run it).
