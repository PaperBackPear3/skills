...
Use this prompt with the `general-purpose` agent type when the eks-updater skill needs to fetch
...
```
You are researching upgrade changelogs for an EKS cluster update. For each package below,
...
- Do not edit files.
- Do not run kubectl/helm/terraform/aws commands.
